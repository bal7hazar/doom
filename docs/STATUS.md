...
> 2026-09-16 : **première vraie preuve du moteur courant par le nœud** (7 tics, 2,29 M steps, 37 s à 4 threads / 76 s mono, 12,8–13,7 Go, vérifiée par leaf-verify) ; task hash `0x5c3f9de0…` confirmé nativement ; calibration ≈ 16 s/M steps à 4 threads, ≈ 9 Go + 2 Go/M steps. Conséquences : C3 ≈ 1 h à 1 h 30 par partie sur un nœud 64 Go (pas 10 min) ; preuve log21 → route registre `doom_21` à construire (dernier maillon crypto).
> 2026-09-16 : **identité migrée vers le moteur O1+O3** (`d0c744d`) : task hash `0x5c3f9de0…` mesuré avec le runtime WASM construit nativement, pins `af4b4ef`, VM R5 pin = build Linux `70b305fe…` (non reproductible entre hôtes) ; e2e Cairo réels verts (9/9 + banc), client 333 tests. D36 : cible D29 indicative, plafond 120 000 bloquant ; régression de jeu verte sur les 26 cas des deux profils en CI.
> 2026-09-16, **premier point C1 mobile** (iPhone iOS 18.7, Worker Cairo réel, moteur `ee5f819`, tunnel HTTPS, `/?bench=1`) : **33,7 tics/s soutenus** sur 700 tics (VM 15,4 ms/tic moy., p95 23,5 ms, max 64 ms ; 22 tics hors budget, 29 abandonnés), rafale 50 tics/s, rendu 59,7 fps à 0,47 ms CPU/frame, 456 Mo WASM ; steps/tic du banc 101 534 moy. / p95 137 372. Verdict « 35 soutenus » manqué de 4 % avant O1+O3 (−22 % de steps) ; à rejouer après migration d'identité. Android non mesuré.
> 2026-09-16 : **tour complet engagement → preuve par un second compte → prime réglée → reclaim validé sur devnet** (`commit_drive.py`, `devnet.test.ts`) ; nœud prouveur validé sur le vrai moteur (27 segments jusqu'à EXIT, goldens reproduits) ; ABI compilée conforme aux décodeurs ; script de migration d'identité prêt. Restent : calibration native, cadence mobile réelle, migration d'identité, Sepolia.
> **D35 prouveur ouvert** décidé et exécuté le 2026-09-15 sur `claude/happy-knuth-pr0xjr` : `DoomRuns` engagements/primes (81 tests), `infra/prover-node` (43 tests), contrôles tactiles + banc mobile, engagement de fin de partie et classement « en attente » (client 323 tests, indexeur 36). Aucune preuve ni transaction réelle ; calibration native et cadence mobile à mesurer par le sponsor.
> Session distante 2026-09-15 sur `claude/happy-knuth-pr0xjr` : correctifs client, test psprites, **écran de coût P4.3 raccordé (272 tests client)**, profil D2 exact (`docs/design/d2-profile.md`). Conclusion : D2 inatteignable sans changer le gameplay ; **C3 ≤ 10 min inatteignable avec le coût fixe par segment (~2,02 M steps)**, arbitrage demandé au sponsor. **O1 + O3 fusionnées (`945143e`, `d1d6577`) : 48 544 → 37 785 steps/tic (−22 %), p99 110 220, 108 976 mots, équivalence exacte ; identité client à migrer (SHA `84c9bae1…`).**
> Audit distant 2026-09-15 : `codex/game-integration` (`310d5d8`) désormais poussée ; sur Linux x86_64 indépendant, **553/554 tests Cairo verts** (un test > 15 Go non exécutable), `run_segment` proving **107 018 mots** et SHA `18100435…` re-mesurés à l'identique, client 252 verts / 10 ignorés, REUSE 1 789/1 789. Commits Codex de `main` et des vagues S14/animation revus : OK, doutes non bloquants dans le handoff. Aucune CI ne tourne sur cette branche (push `main`/PR seulement).
