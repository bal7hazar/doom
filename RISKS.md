...
coûte 3,07 % de plus, toujours sous son seuil. La distribution complète donne
49 524,87 steps moyens et p99 125 157 sur 2 946 tics, soit −6,99 % moyen face
à D29 remesurée avec le même protocole ; D2 reste dépassé. Preuve courte root valide en **41,553 s**, toujours **log21** et environ
...
| R1-A3 | Build WASM64 et mesure dans Chrome sur 3 machines. **Fait sur M2 Max (S2) : 2^20 steps en 35,6 s et 3,05 GiB, mono-thread, vérifié dans le navigateur ; build reproductible bit à bit.** Reste : machines 16 GB (Apple Silicon, x86 Windows). | S2 fait / Phase 3 | 5 jp | 2^20 steps : < 12 GB et < 120 s sur les trois machines ; aucune éviction d'onglet. |
...
| R1-A8 | **Blocages intermittents avec threads** sur des segments ≥ 2 M steps dans Chrome (2 sur ~16 runs en P3.1 ; jamais sous Node 24) : diagnostiquer (pré-croissance mémoire, allocateur std sous `+atomics`), et en attendant **segments ≤ 1,5 M steps par défaut avec threads**, timeout par segment et **reprise mono-thread** automatique. | Phase 3 (P3.2) | 3 jp | 0 blocage sur 50 preuves de 1,5 M steps ; taux d'échec < 1 %. |
...
| R2-A5 | ~~Dédup inter-cellules~~ **Rejetée (S1 : ×4,6 sur un hitscan).** Remplacée par R2-A10. | Clos | — | — |
| R2-A9 | Accélérateur « cellule blockmap → liste de sous-secteurs » généré par l'extracteur WAD (descente BSP 1 050 → ~150 steps ; −5 400 steps/tic projetés). | Phase 1 (`tools/wad`, `doom_map`) | 3 jp | Localisation de secteur ≤ 200 steps dans le cas courant. |
| R2-A10 | Itération blockmap sans matérialisation de tableau (spans sur constantes, −1 000 steps/tic). | Phase 1 (`blockmap`) | 2 jp | Zéro allocation dans `P_TryMove`. |
| R2-A11 | Cellule blockmap conservée dans l'état du mobj et mise à jour incrémentalement (−900 steps/tic). | Phase 1 (`doom_physics`) | 1 jp | Mesuré sur les replays. |
| R2-A12 | **Budget de taille de bytecode** : S1 mesure 98 585 mots pour le prototype → 1,45 M steps de hachage bootloader par segment (non prouvable). Cible **< 32 000 mots** (≈ 470 k steps, 45 % d'un segment) et idéalement < 16 000 ; règle de packing : packer si `14,7 × felts_économisés / K > steps_supplémentaires × accès` ; test CI de taille de bytecode ; **S4b : poseidon validé (1 969 + 5,5 × mots) ; le lifting 2^21 est inatteignable mais inutile : les segments peuvent dépasser 2^20 steps (1,6 M testé, 5,4 M sans changer de lifting) tant que le plus gros composant AIR reste ≤ 2^20 lignes → mesurer les composants de `doom_run` et dimensionner K par la RSS navigateur.** | Phase 1, test CI dès P1.5 | 3 jp | Bytecode de `doom_run` sous budget à chaque PR. |
...
| R3-A7 | **Drive devnet** systématique (starknet-devnet 0.9+, reçus réels) pour toutes les tx, avec assertion « ≤ 90 % du plafond invoke » ; re-jouer à chaque bump de version Starknet. | S5/Phase 4 | 2 jp | Script `infra/devnet_drive.py` en CI nocturne. (P4.0 : `cairo/doom_contracts/tools/devnet_drive.py` + `check_registry.sh` sont la base ; snforge doit tourner en `tracked_resource = "cairo-steps"`, seul mode qui reproduit les reçus devnet à 3 % près.) |
...
| R7-A6 | **Transport calldata plutôt que storage** : S5 chiffre le staging des preuves S4 (94–96 k felts) à ≈ 4,3e9 L2 gas sur 5 tx contre ≈ 6,8e7 gas si les sections sont passées en calldata aux phases de vérification (61×). Concevoir le vérifieur résumable « N phases, sections en calldata liées par digest » (design lane 2 de modeofO, `fri_transport.cairo`) avant la Phase 4 ; objectif ≤ 6 invokes et ≤ 2,5e9 L2 gas par fait. **P4.0 (2026-09-12, `docs/design/onchain-verifier.md`) : prototype livré et mesuré sur devnet — 5 tx (pire tx 90,4 % du plafond) ou 6 tx (84 %), 3,81e9 L2 gas ≈ 116 STRK/fait au prix S5 (2,15× moins que l’extrapolation stockée), 0 felt de preuve en storage ; la cible 2,5e9 est manquée à cause du repli FRI en QM31 naïf (48 % du coût) → levier P4.1.** | Phase 4 (avant P4.1) | 10 jp | Drive devnet : total L2 gas par fait publié, chaque tx ≤ 90 % du plafond. |
...
| R11-A2 | Différences avec upstream maintenues comme **patches** (`prover/wasm/patches/*.patch`, `git apply` en CI) plutôt que fork long ; PR upstream pour chaque patch. **P3.1 : 6 patches (4 pour le mono-thread, +2 pour les threads : `std` `no_threads` sur wasm64, `parking_lot_core`), chacun avec un brouillon de PR ; plafond relevé à 8 tant que chaque patch a sa PR upstream.** | S2/Phase 3 | 2 jp + suivi | Liste de patches ≤ 8, chacun avec une PR upstream ouverte. |
