...
| R1-A2 | Profil mémoire du prouveur natif (`/usr/bin/time -l`, `heaptrack`/`dhat` sur Linux) pour 2^18, 2^19, 2^20 avec `canonical_small` : séparer trace principale, interaction, FRI, Merkle. | S0 | 2 jp | Courbe RSS = f(log_steps) documentée ; part fixe < 3 GB. |
...
| R1-A4 | Dimensionner K (tics/segment) pour viser **2^19 steps utiles** (marge ×2 sous le plafond 2^20 du `CanonicalSmall`), bootloader (~30 k steps) et hash d'état inclus ; K calculé dynamiquement depuis le compteur de steps réel plutôt que fixe. | S1→Phase 3 | 2 jp | Aucun segment n'excède 2^20 sur les 20 replays dorés ; répartition de taille publiée. |
...
| Id | Action | Quand | Effort | Critère de sortie |
|----|--------|-------|--------|-------------------|
| R4-A1 | S0 : reproduire les deux paniques avec le prouveur du monorepo ; si elles persistent, ouvrir des issues upstream avec fixtures minimales ; consigner le verdict dans `CONTEXT.md`. | S0 | 1 jp | Verdict écrit ; règles A7 confirmées ou assouplies. |
...
| Id | Action | Quand | Effort | Critère de sortie |
|----|--------|-------|--------|-------------------|
| R7-A1 | Estimation par `starknet_simulateTransactions` sur la **séquence complète** (stage → phase1 → phase2 → submit) avec `skip_validate` ; bornes = estimation × 1,15 pour L2 gas, ×1,3 pour L1 data gas ; jamais de multiplicateur global ×1,5. | S5/Phase 4 | 3 jp | Écart estimation/réel < 20 % sur 10 soumissions (**C5**). |
