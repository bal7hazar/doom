...
**Does not** (yet): the `DoomRuns` consumer (recomposition of the leaf outputs, run records,
leaderboard) — its interface is specified in the design document §6.
...
| `tools/` | `emit_calldata.py` (proof → per-transaction packed calldata), `devnet_drive.py` (declare, deploy, drive, receipts, pricing), `trace_tx.py` (per-call gas of a tx), `devnet_probe.py` (transport calibration), `gen_multiverifier_consts.py`, `check_registry.sh` |
| `results/` | devnet receipts of the 5-tx and 6-tx drives, deployment (class hashes, declare gas), transport probe |
