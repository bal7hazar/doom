...
mod tests {
    ...
    use stwo_constraint_framework_ref::AirComponent;
    #[allow(unused_imports)]
    use stwo_constraint_framework_ref::test_utils::{make_interaction_trace, preprocessed_mask_add};
    #[allow(unused_imports)]
    use stwo_constraint_framework_ref::{
        LookupElementsTrait, PreprocessedMaskValues, PreprocessedMaskValuesTrait,
    };
    use stwo_verifier_core_ref::fields::qm31::{QM31, QM31Impl, QM31Trait, qm31_const};
    ...
    fn test_evaluation_result() {
        ...
        let mut preprocessed_trace = preprocessed_mask_add(
            preprocessed_trace,
            BITWISE_XOR_9_0_IDX,
            qm31_const::<1052671046, 1968306984, 497428152, 1503133367>(),
        );
        let mut preprocessed_trace = preprocessed_mask_add(
            preprocessed_trace,
            BITWISE_XOR_9_1_IDX,
            qm31_const::<985561867, 1834089256, 430319288, 1503133367>(),
        );
        let mut preprocessed_trace = preprocessed_mask_add(
            preprocessed_trace,
            BITWISE_XOR_9_2_IDX,
            qm31_const::<918452688, 1699871528, 363210424, 1503133367>(),
        );
        ...
        assert_eq!(sum, QM31Trait::from_fixed_array(VERIFY_BITWISE_XOR_9_SAMPLE_EVAL_RESULT))
    }
}
