...
use bounded_int_ref::{AddHelper, BoundedInt, SubHelper, upcast};
...
pub impl M31Add of core::traits::Add<M31> {
    ...
    fn add(lhs: M31, rhs: M31) -> M31 {
        let sum = bounded_int_ref::add(lhs.inner, rhs.inner);
        match bounded_int_ref::constrain::<BoundedInt<0, { 2 * (P - 1) }>, P>(sum) {
            Ok(lt) => M31Trait::new(lt),
            Err(gte) => M31Trait::new(upcast(bounded_int_ref::sub(gte, M31_P))),
        }
    }
}
...
pub impl M31Sub of core::traits::Sub<M31> {
    ...
    fn sub(lhs: M31, rhs: M31) -> M31 {
        let diff = bounded_int_ref::sub(lhs.inner, rhs.inner);
        let res = match bounded_int_ref::constrain::<BoundedInt<{ -(P - 1) }, { P - 1 }>, 0>(diff) {
            Ok(lt) => upcast(bounded_int_ref::add(lt, M31_P)),
            Err(gte) => gte,
        };
...
pub impl M31AddConstrainP of bounded_int_ref::ConstrainHelper<BoundedInt<0, { 2 * (P - 1) }>, P> {
    type LowT = M31InnerT;
    type HighT = BoundedInt<{ P }, { 2 * (P - 1) }>;
}


pub impl M31SubConstrain0 of bounded_int_ref::ConstrainHelper<BoundedInt<{ -(P - 1) }, { P - 1 }>, 0> {
