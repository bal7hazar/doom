...
//!
//! Reference vectors come from `scripts/model.py`, an independent Python
//! transcription of `p_user.c`, `p_pspr.c` and `p_inter.c` (never from this
//! code); everything geometric runs on the real Freedoom E1M1 through
//! `doom_physics`.
...
use prng::from_index;
use super::env::{Env, PlayerEvent, env_of};
use super::inter::{absorb, damage_player, give_ammo, touch_special};
use super::state::{
...
use super::weapon::{PS_WEAPON, S_PLAY, chain, check_ammo, drop_weapon, move_psprites, set_psprite};
...
fn test_psprite_script() {
    ...
    p.psp_sy = Fixed { enc: BIAS + super::state::WEAPONTOP };
...
fn test_check_ammo_falls_back_in_dooms_order() {
    ...
    q.psp_sy = Fixed { enc: BIAS + super::state::WEAPONTOP };
    ...
    r.psp_sy = Fixed { enc: BIAS + super::state::WEAPONTOP };
...
fn test_a_weapon_switch_lowers_then_raises() {
    ...
    assert(p.psp_sy == Fixed { enc: BIAS + super::state::WEAPONTOP }, 'at the top');
}
...
fn test_view_height_stays_between_its_bounds() {
    ...
    while tic != 120 {
        ...
        assert(
            fixed::ge(p.viewheight, Fixed { enc: BIAS + super::state::HALF_VIEWHEIGHT }),
            'never below VIEWHEIGHT/2',
        );
        assert(fixed::le(p.bob, Fixed { enc: BIAS + super::state::MAXBOB }), 'bob capped');
        tic += 1;
    }
}
...
fn test_serialization_schema() {
    ...
    assert(super::state::fields() == PLAYER_FELTS, 'fields agrees');
    let felts = out.span();
    let mut i: u32 = 0;
    while i != felts.len() {
