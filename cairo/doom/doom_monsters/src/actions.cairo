...
use super::{Ctx, Patch, SIGHT_TTL, read_mobj};
...
fn p_move(
    ...
    let v: Verdict = try_move(ctx.w, mobjs, ref g, ref mo, me, tryx, tryy, ref moves);
    if !v.ok {
...
fn try_walk(
    ...
    if !p_move(ctx, mobjs, ref g, ref mo, me, ref ev) {
        return false;
    }
...
fn new_chase_dir(
    ...
    if d1 != DI_NODIR && d2 != DI_NODIR {
        ...
        if mo.move_dir != turnaround && try_walk(ctx, mobjs, ref g, ref rng, ref mo, me, ref ev) {
            return;
        }
    }
    ...
    if d1 != DI_NODIR {
        mo.move_dir = d1;
        if try_walk(ctx, mobjs, ref g, ref rng, ref mo, me, ref ev) {
            return;
        }
    }
    if d2 != DI_NODIR {
...
pub fn check_melee_range(ctx: Ctx, ref mo: Mobj, target: @Mobj) -> bool {
    ...
    check_sight_cached(ctx.w, ref mo, target, ctx.tic, SIGHT_TTL)
}
...
pub fn check_missile_range(ctx: Ctx, ref rng: Prng, ref mo: Mobj, target: @Mobj) -> bool {
    if !check_sight_cached(ctx.w, ref mo, target, ctx.tic, SIGHT_TTL) {
        return false;
    }
...
fn hears(ctx: Ctx, listener: u32, noise: u32) -> bool {
    !reject_of(ctx.w.map.reject, ctx.w.map.reject_stride, ctx.w.map.pow2, listener, noise)
}
...
pub fn look_for_players(ctx: Ctx, mobjs: Span<Mobj>, ref mo: Mobj, all_around: bool) -> bool {
    let n = ctx.players.len();
    let mut k: u32 = 0;
    let mut found = false;
    while k != n {
        let pi = *ctx.players.at(k);
        k += 1;
        let p = mobjs.at(pi);
        if *p.health <= 0 {
            continue;
        }
        if !check_sight_cached(ctx.w, ref mo, p, ctx.tic, SIGHT_TTL) {
            continue;
        }
        if !all_around {
...
fn pick_sound(ctx: Ctx, ref rng: Prng, base: u32, low: u32, high: u32, span: u8) -> u32 {
...
pub fn a_look(
    ...
    let src = ctx.noise.source;
    if src != NO_MOBJ && src < mobjs.len() && hears(ctx, mo.sector, ctx.noise.sector) {
        let targ = mobjs.at(src);
        if has(*targ.flags, MF_SHOOTABLE) {
            mo.target = src;
            if has(mo.flags, MF_AMBUSH) {
                seeyou = check_sight_cached(ctx.w, ref mo, targ, ctx.tic, SIGHT_TTL);
            } else {
                seeyou = true;
            }
        }
    }
    if !seeyou && !look_for_players(ctx, mobjs, ref mo, false) {
        return fsm::NO_ACTION;
    }
    let base = sound_of(MI_SEESOUND.span(), mo.kind);
    if base != SFX_NONE {
        let mut s = pick_sound(ctx, ref rng, base, SFX_POSIT1, SFX_POSIT3, 3);
        if s == base {
            s = pick_sound(ctx, ref rng, base, SFX_BGSIT1, SFX_BGSIT2, 2);
        }
        ev.append(sound(me, s));
    }
    ev.append(event(EV_WAKE, me, mo.target, 0));
    set_state(ctx.w, ref mo, seestate_of(mo.kind))
}
...
pub fn a_chase(
    ...
    if !has_target || !has(target.flags, MF_SHOOTABLE) {
        ...
        if look_for_players(ctx, mobjs, ref mo, true) {
            return fsm::NO_ACTION;
        }
        return set_state(ctx.w, ref mo, spawnstate_of(mo.kind));
    }
    // Do not attack twice in a row.
    if has(mo.flags, MF_JUSTATTACKED) {
        mo.flags = without(mo.flags, MF_JUSTATTACKED);
        new_chase_dir(ctx, mobjs, ref g, ref rng, ref mo, me, @target, ref ev);
        return fsm::NO_ACTION;
    }
    // Melee.
    let melee = meleestate_of(mo.kind);
    if melee != 0 && check_melee_range(ctx, ref mo, @target) {
        let s = sound_of(MI_ATTACKSOUND.span(), mo.kind);
        if s != SFX_NONE {
            ev.append(sound(me, s));
        }
        return set_state(ctx.w, ref mo, melee);
    }
    // Missile. Skill 2 is below nightmare, so `movecount` gates it.
    let missile = missilestate_of(mo.kind);
    if missile != 0 && mo.move_count == 0 && check_missile_range(ctx, ref rng, ref mo, @target) {
        mo.flags = mo.flags | MF_JUSTATTACKED;
        return set_state(ctx.w, ref mo, missile);
    }
...
pub fn a_face_target(ctx: Ctx, ref rng: Prng, ref mo: Mobj, target: @Mobj) {
    ...
    if has(*target.flags, MF_SHADOW) {
        let (next, spread) = rng.sub_random(ctx.w.rndtable);
        rng = next;
...
fn spread_angle(ctx: Ctx, ref rng: Prng, base: Angle) -> Angle {
    let (next, spread) = rng.sub_random(ctx.w.rndtable);
    rng = next;
    let s: felt252 = spread.into();
...
fn shoot(
    ...
    let hit = line_attack(ctx.w, mobjs, ref g, me, angle, MISSILERANGE, slope);
    match hit {
        Hit::Nothing => {},
        Hit::Wall((
            _, p, _,
        ...
        Hit::Thing((
            ...
            hurt(ctx, mobjs, ref rng, idx, me, me, damage, ref patches, ref ev);
        },
    }
}
...
pub fn hurt(
    ...
    let out: DamageOutcome = damage_mobj(
        ctx.w, mobjs, ref rng, ref t, target_idx, inflictor, source, damage, true,
    );
    passive(ctx, ref rng, ref t, target_idx, out.action, ref ev);
    if out.counts_kill {
...
pub fn passive(
    ...
    if action == doom_things::tables::A_PAIN {
        ...
        a_scream(ctx, ref rng, ref mo, me, ref ev);
...
pub fn a_scream(ctx: Ctx, ref rng: Prng, ref mo: Mobj, me: u32, ref ev: Array<MonsterEvent>) {
    ...
    let mut s = pick_sound(ctx, ref rng, base, SFX_PODTH1, SFX_PODTH3, 3);
    if s == base {
        s = pick_sound(ctx, ref rng, base, SFX_BGDTH1, SFX_BGDTH1 + 1, 2);
    }
    ev.append(sound(me, s));
}
...
pub fn a_pos_attack(
    ...
    a_face_target(ctx, ref rng, ref mo, @target);
    let base = mo.angle;
    let aim: Aim = aim_line_attack(ctx.w, mobjs, ref g, me, base, MISSILERANGE);
    ev.append(sound(me, SFX_PISTOL));
    let angle = spread_angle(ctx, ref rng, base);
    let damage = roll_damage(ctx, ref rng, 5, 3);
    shoot(ctx, mobjs, ref g, ref rng, me, angle, aim.slope, damage, ref patches, ref ev);
}
...
fn roll_damage(ctx: Ctx, ref rng: Prng, n: u8, mul: u32) -> u32 {
    let (next, roll) = rng.next(ctx.w.rndtable);
    rng = next;
...
pub fn a_troop_attack(
    ...
    let target = read_mobj(mobjs, patches.span(), mo.target);
    a_face_target(ctx, ref rng, ref mo, @target);
    if check_melee_range(ctx, ref mo, @target) {
        ev.append(sound(me, SFX_CLAW));
        let damage = roll_damage(ctx, ref rng, 8, 3);
        hurt(ctx, mobjs, ref rng, mo.target, me, me, damage, ref patches, ref ev);
        return;
    }
    // Launch a missile.
    let mut moves: Array<MoveEvent> = array![];
    ...
    let (missile, _) = spawn_missile(
        ctx.w, mobjs, ref g, ref rng, @mo, me, @target, FIREBALL, idx, ref moves,
    );
...
pub fn a_sarg_attack(
    ...
    let damage = roll_damage(ctx, ref rng, 10, 4);
    hurt(ctx, mobjs, ref rng, mo.target, me, me, damage, ref patches, ref ev);
}
