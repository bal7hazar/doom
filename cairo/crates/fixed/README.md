...
| `neg` | 2 | 0 | |
| `from_units` | 3 | 0 | |
| `ge` / `gt` / `le` / `lt` | 11 | 2 | one `felt_ge` |
| `min` + `max` (both) | 25 | 4 | 13 each |
| `abs` | 14 | 2 | |
| `magnitude` / `is_neg` | 15 | 2 | |
| `to_units` | 15 | 5 | one `u128` division (floor) |
| `shr8` | 16 | 5 | C's `>> 8` on a signed value (Doom pre-shifts in `P_InterceptVector`) |
| `split` (sign + magnitude) | 16 | 2 | one sign test instead of two |
| `mul` | 18 | 5 | exactly S1 §5.1's figure for bare magnitudes, sign handling included |
| `div` | 77 | 12 | S1 quotes 55 for bare magnitudes; +22 for the two sign tests and Doom's overflow guard |
| `add(x, mul(v, c))` (momentum step) | 21 | 5 | representative composite |

Bytecode: **2 681 words** for the benchmark executable, crate included
...
`scarb test -p fixed` — 26 unit tests (`src/tests.cairo`) plus 2 integration
