...
(`ge`/`gt`/`le`/`lt`/`min`/`max`/`is_neg`), operator sugar for all of them,
...
Bytecode: **2 562 words** for the benchmark executable, crate included
...
`scarb test -p fixed` — 25 unit tests (`src/tests.cairo`) plus 2 integration
...
* **edge cases**: zero, `±1` ulp, `MAXINT`/`MININT`, division by zero and the
  ...
  its domain (must panic), `to_units` flooring;
