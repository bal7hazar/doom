...
export class GameInput {
  ...
  clear(): void { this.keys.clear(); this.mouseX = 0; this.fire = false; this.weapon = undefined; }
  sample(): number {
    ...
    const fast = down("ShiftLeft", "ShiftRight");
    const forward = (down("KeyW", "ArrowUp") - down("KeyS", "ArrowDown")) * (fast ? 50 : 25);
    const side = (down("KeyD") - down("KeyA")) * (fast ? 40 : 24);
    const turn = (down("ArrowLeft") - down("ArrowRight")) * (fast ? 1280 : 640) - this.mouseX * 32;
    // d_event.h bits, as consumed by doom_player::state/think (not compact WeaponId).
    let buttons = Number(this.fire || Boolean(down("ControlLeft", "ControlRight"))) | (down("KeyE", "Space") << 1);
    if (this.weapon !== undefined) buttons |= 4 | (this.weapon << 3);
