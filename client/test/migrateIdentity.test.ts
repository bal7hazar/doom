...
describe("migrate-identity.mjs", () => {
  ...
  it("rewrites the pins, freezes the retired identity, refreshes the README table and reports before/after", async () => {
    ...
    expect(read("pins")).toMatch(/^\/\*\* Cairo proving executables at aaaaaaa \(explicit migration from ee5f819\)/);
    ...
    expect(read("readme")).toContain("| Retired identities (refused, `legacyDoomIdentity.ts`) | `0c8a3a8`, `ee5f819` |");
